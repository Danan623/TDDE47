...
#endif
//static struct list sleeping_list; // list of sleeping threads (lab2)
...
static struct list sleeping_list; //Added in lab 2
...
timer_init (void) 
{
  ...
  //sleeping_list = malloc(sizeof(struct list));
...
/**
 * comparator function for list_insert_ordered. argument a and b are list elements 
...
bool comparator(const struct list_elem *a, const struct list_elem *b, void *aux) {
  ...
  // check threads wakeup time. If first thread has a lower wakeup time than second thread, return true 
...
{
  if (ticks <= 0){// if ticks is less than or equal to 0, no need to sleep.
    return;
  } 
  ...
  //ASSERT(!intr_context());
  int old_level = intr_disable(); // disable interrupts (old_level is used to restore interrupts later)

  struct thread *thread = thread_current();// get the current thread
...
timer_interrupt (struct intr_frame *args UNUSED)
{
  ...
  thread_tick ();//  call thread_tick() to update the thread's recent_cpu value
