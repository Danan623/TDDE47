...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  for (e = list_begin(list); e != list_end(list); e = list_next(e)) { //Loop through child_list
    ...
    if (family->child->tid == child_tid) { //If child_tid is found in child_list
...
process_exit (void)
{
  ...
  if (family != NULL) {
    ...
    family->alive_count--;
    lock_release(&family->lock);//Release lock
