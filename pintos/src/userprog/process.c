...
process_execute (const char *file_name) 
{
  ...
  tid_t tid; //Thread ID of new thread.

  /* Make a copy of FILE_NAME.
     Otherwise there's a race between the caller and load(). */
  fn_copy = palloc_get_page (0); 
  if (fn_copy == NULL)  //If no memory available,
    return TID_ERROR; 
  strlcpy (fn_copy, file_name, PGSIZE); //Copy file_name to fn_copy.
 
  /* Create a new thread to execute FILE_NAME. */
  struct parent_child *relationship = malloc(sizeof(struct parent_child)); 
  sema_init(&relationship->sema, 0); //Start at 0 so we let start_process finish with sema_up before we reach sema_down, exec and start process
  sema_init(&relationship->sema_wait, 0); //To know if child has exited, for our wait function, used in exit and wait
  lock_init(&relationship->lock); //To prevent race conditions in alive_count
  relationship->file_name = fn_copy; //Save file name for what is needed in start_process
  relationship->exit_status = -1; //If thread creation fails, return -1
  relationship->alive_count = 2; //Alive count is 2, one for parent and one for child
  relationship->has_waited = false;
  relationship->load_success = false;
  ...
  relationship->child_tid = tid;
  return tid; //Return child's tid 
}
...
{
  struct parent_child *relationship = (struct parent_child*) p; 
  relationship->child = thread_current(); //Running on child thread.
  thread_current()->family = relationship;  //Add relationship to child thread.

  char *file_name = relationship->file_name; //Get file name from relationship 
  ...
  /* Start the user process by simulating a return from an
     ...
     and jump to it. */
  asm volatile ("movl %0, %%esp; jmp intr_exit" : : "g" (&if_) : "memory"); // set up main stack I think...
  NOT_REACHED (); // Should never reach this point
}
...
process_wait (tid_t child_tid UNUSED) 
{
  ...
  struct list *list = &cur->child_list; //Get child_list from current thread
  struct list_elem *e; //Create list_elem
  struct parent_child *family; //Create parent_child

  for (e = list_begin(list); e != list_end(list); e = list_next(e)) { //Loop through child_list
    family = list_entry(e, struct parent_child, list_elem); //Get relationship from list_elem
    if (family->child_tid == child_tid) { //If child_tid is found in child_list
      break; //Then we have our correct family, so breaak our search
      }
      family = NULL; //If not, reset family so we dont trigger the next if statement
    }
    if (family != NULL) {
      lock_acquire(&family->lock); //Protect alive_count from race conditions
      int alive_count = family->alive_count;
      lock_release(&family->lock);
      if (alive_count == 2) {
        sema_down(&family->sema_wait); //Will wait for the child thread to have exited
      }
      status = family->exit_status; //Get status to return
      family->exit_status = -1; //Change stored status in family
    } 
  return status; //If child_tid is not found in child_list return -1
...
process_exit (void)
{
  ...
  if (family != NULL) {
    lock_acquire(&family->lock); //Protect from race condition
    family->alive_count--;
    lock_release(&family->lock);
    if (family->alive_count == 0) {
      free(family);
    }
    else { //Must be done in else or we get error cause family is freed
      sema_up(&family->sema_wait); //Thread exits so its parent is ok to continue
    }
  }
...
load (const char *file_name, void (**eip) (void), void **esp) 
{
  ...
  if (!setup_stack (esp)){
    ...
    //CODE FOR LAB 4 BELOW
  char *token, *save_ptr; // used by strtok_r to keep track of where it is in the string
  char *argv[32]; // array of pointers to the tokens
  int argc = 0;
  int padding = 0;
  //FOR LOOP inserts tokens into array.
  for (token = strtok_r (file_name, " ", &save_ptr); token != NULL;
        token = strtok_r (NULL, " ", &save_ptr)){
          argv[argc] = token; // store the address of the token in the array
  ...
  //FIX PADDING
  padding = ((unsigned)*esp) % 4; // Adjust the stack pointer so it is divisible by 4 (från lab kompendiet)
  ...
  *esp -= sizeof(int); // move the stack pointer
  ...
  //pusha en dummy return address till stacken
  *esp -= sizeof(void*); // move the stack pointer
